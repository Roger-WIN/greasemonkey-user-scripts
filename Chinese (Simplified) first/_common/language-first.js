...
function convert(lang_target) {
    let index = lang_target.length;
...
/* 没有需要排除的情况 */
function convertWithoutExclude(lang_target) {
...
/* 有需要排除的情况 */
function convertWithExclude(lang_target, flags_exclude) {
    if (pathName.startsWith(lang_target)) // 网页已转换
        return; // 退出函数，避免重复转换
